...
def subdomain(domain):
        ...
        os.system("curl -ss https://dns.bufferover.run/dns?q={} | jq '.FDNS_A[]' | sed 's/^\".*.,//g' | sed 's/\"$//g' | sort -u | grep -i {} > ~/recon/{}/bufferover.txt".format(domain,domain,domain))
        ...
        if os.path.isfile("~/recon/"+domain+"/merge/unique.txt"):
...
def screenshots(domain):
        ...
        os.system("cd  ~/recon/"+domain+"/screenshots/ ; cat ~/recon/"+domain+"/uphost/uphost.txt | aquatone -threads 4 -ports large -chrome-path $(which chromium-browser)")
...
def directories(domain):
        ...
        with open('~/recon/{}/uphost/uphost_tmp.txt'.format(domain)) as myfile:
        ...
        for i in range(num_lines):
                os.system("cd ~/recon/"+domain+"/directories/ ; python3 ~/tools/dirsearch/dirsearch.py -u https://"+lines[i]+" -e * -x 400,403,406,409,410,412,415,418,423,426,431,450,429,500,503,506,509,598 -t 100 -b --simple-report="+lines[i]+".txt")
...
def uphost(domain):
        ...
        os.system("cat ~/recon/"+domain+"/uphost/uphost_tmp.txt | httpx -threads 200 -silent > ~/recon/"+domain+"/uphost/uphost.txt")
...
def gau(domain):
        ...
        os.system("cat ~/recon/{}/uphost/uphost_tmp.txt | gau >> ~/recon/{}/gau/gau.txt".format(domain,domain))
...
def nuclei(domain):
        ...
        os.system("cat ~/recon/{}/uphost/uphost.txt | nuclei -t ~/tools/nuclei-templates/cves/ -o ~/recon/{}/nuclei/cves.txt".format(domain,domain))
        ...
        os.system("cat ~/recon/{}/uphost/uphost_tmp.txt | nuclei -t ~/tools/nuclei-templates/subdomain-takeover/ -o ~/recon/{}/nuclei/subdomain-takeover.txt".format(domain,domain))
        ...
        os.system("cat ~/recon/{}/uphost/uphost.txt | nuclei -t ~/tools/nuclei-templates/files/ -o ~/recon/{}/nuclei/files.txt".format(domain,domain))
        ...
        os.system("cat ~/recon/{}/gau/gau.txt | nuclei -t ~/tools/nuclei-templates/tokens/ -o ~/recon/{}/nuclei/tokens.txt".format(domain,domain))
        ...
        os.system("cat ~/recon/{}/uphost/uphost.txt | nuclei -t ~/tools/nuclei-templates/vulnerabilities/ -o ~/recon/{}/nuclei/vulnerabilities.txt".format(domain,domain))
        ...
        os.system("cat ~/recon/{}/uphost/uphost.txt | nuclei -t ~/tools/nuclei-templates/panels/ -o ~/recon/{}/nuclei/panels.txt".format(domain,domain))
